...
const CheckInventoryDetail = () => {
    ...
    React.useEffect(() => {
        axios
            .get(`${apiBaseUrl}/inventory/check_line?checkCode=${code}`, {
                headers: {
        ...
        axios
            .get(`${apiBaseUrl}/inventory/check_inventory/code?code=${code}`, {
                headers: {
                    // token: Cookies.get('token'),
                    Authorization: getCookie('Authorization'),
                },
        ...
        <Box>
            ...
            >
                <Grid container spacing={3} display="flex" alignItems="center">
                    ...
                    <Grid
                        backgroundColor={checkInvetoryRequest.status === 'Đã cập nhập số lượng' ? 'aqua' : '#e49c06'}
                        borderRadius="25px"
                    >
                ...
                {checkInvetoryRequest.status === 'Đang kiểm hàng' ? (
                    <Grid container spacing={3} display="flex" justifyItems="flex-end" alignItems="center">
                        <Grid>
                            <Button size="large" variant="contained" onClick={handleDeleteRequest}>
                                Xóa
                            </Button>
                        </Grid>
                        <Grid>
                            <Button size="large" variant="contained" onClick={handleUpdateProductQuantity}>
                                Cân Bằng kho
                            </Button>
                        </Grid>
                    </Grid>
            ...
            <Box mt={4}>
                <CheckInventoryRequestDetail rows={checkLines} />
            </Box>
        </Box>
    );
};
